...
Reff_inc_strat <- readRDS(file="output/incidence_four_cities_2.2_200sims.rds")
gc()
inc_daily <-Reff_inc_strat %>% 
  unnest(inc) %>% 
  drop_na() %>%
  mutate(inc_sum=future_pmap( .f=summarise_projections,.l=list(inc,cumulative=F),.progress = T)) %>% 
  select(-inc) %>% 
  unnest(inc_sum) %>% 
  group_by(PYNAME,travel_restrictions,chunyun,Reff,date) %>% 
...
# quantile_cities <- readRDS("data/china_shp.rds") %>% 
...
#            CNTY_CODE==211300|
#            CNTY_CODE==542500) %>% 
#   mutate(quant=factor(x=c("25%","75%","50%","97.5%","2.5%"),levels = c("2.5%","25%","50%","75%","97.5%")))
...
inc_cumulative %>%
  ...
  group_by(PYNAME,Reff,chunyun,travel_restrictions) %>% 
  filter(Reff=="2.2",travel_restrictions=="Travel restrictions",chunyun=="Chunyun") %>% 
...
inc_cumulative%>%
  ...
  ggsave(
    filename = "output/cases_1_march_inc2.5.png",
    width = 210,#210*2,
    height = 148,#297*4,
    dpi = 320,
    units = "mm"
...
inc_cumulative%>%
  ...
  #geom_text(aes(label=formatC(signif(inc_median,3),format="f", big.mark=",", digits=0),y=as.integer(inc_median),x=fct_rev(Reff_reduction)),nudge_x = 0.2,size=2,hjust=0)+
  facet_nested(~stringr::word(PYNAME)+chunyun,nest_line=T) +
  ...
  #scale_colour_viridis_d(name = "", option = "viridis",begin=0.1,end = 0.9) +
  ggsave(
    filename = "output/cases_1_march_inc3.png",
    width = 210,#210*2,
    height = 148,#297*4,
    dpi = 320,
    units = "mm"
...
inc_daily %>% 
  ...
  geom_vline(aes(xintercept=as.Date("2020-01-23"),colour="Travel restriction and other NPIs enacted"))+
  scale_color_manual(name="", values=c("red"))+
  facet_nested(stringr::word(PYNAME)~chunyun+fct_rev(Reff_reduction)) +
  scale_y_log10(labels = trans_format("log10", math_format(10^.x))) +
  scale_x_date(
    breaks = scales::breaks_width("1 month"),
    labels = scales::label_date_short()
  ) +
  coord_cartesian(xlim = c(as.Date("2019-12-01"), as.Date("2020-03-15")),
                  ylim = c(1, 10000)) +
  labs(
  ...
  theme(legend.position = "bottom",
        ...
        axis.ticks = element_line())+
  ggsave(
    filename = "output/Inc_four_cities_log3.png",
    width = 210,#210*2,
    height = 148,#297*4,
    dpi = 320,
    units = "mm"
